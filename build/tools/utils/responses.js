 ...
 */
async function promptModel(modelId, prompt) {
    try {
        let modelInstance;
...
export async function callOpenAI(prompt, model = 'o3-mini') {
    return promptModel(model, prompt);
}
export async function callVertexAI(prompt, model = 'gemini-2.5-pro-exp-03-25') {
    return promptModel(model, prompt);
}
export function cleanCommandResponse(content, defaultCommand) {
