...
export async function analyzeArchitecture(
  ...
  try {
    ...
    try {
      const treeResult = spawnSync('tree', ['--prune', '-I', '.context|.cursor|.backup|public|magicui|ui|node_modules|coverage|test-results|playwright-report|types|assets|pnpm-lock.yaml|package-lock.yaml|*.lock|*.log|*.svg|assets|.next'], spawnOptions);
 ...
 */
async function getRipgrepCommand(task: string, fileStructure: string): Promise<string> {
...
async function getRepomixCommand(task: string, searchOutput: string, fileStructure: string): Promise<string> {
  ...
  const prompt = 
    ...
    `\`\`\`\n\n`+
    `# Respond only with the repomix command. Aim to include about 100 files with the repomix command.\n\n`;
